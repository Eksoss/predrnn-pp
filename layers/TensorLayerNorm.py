...
def tensor_layer_norm(x, state_name):
    ...
    if dims == 4:
        m, v = tf.compat.v1.nn.moments(x, [1,2,3], keep_dims=True)
    elif dims == 5:
        m, v = tf.compat.v1.nn.moments(x, [1,2,3,4], keep_dims=True)
    else:
    ...
    x_tln = tf.compat.v1.nn.batch_normalization(x, m, v, b, s, EPSILON)
    return x_tln
